...
import { getForums } from 'apiSdk/forums';
...
import { SubscriptionListPage as ShowCaseList } from 'pages/subscriptions';

function DashboardPage() {
  const { session } = useSession();

  const forumStat = useSWR<PaginatedInterface<ForumInterface>>(
    () => `/forums`,
    () => getForums({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Forums',
      result: forumStat?.data?.totalCount,
      isLoading: forumStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`subscriptions`} passHref legacyBehavior>
                  <Button
